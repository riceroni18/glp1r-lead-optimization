...
1. **Structural Data Ingestion (`get_seq.py`):** A custom Python parser designed to map and convert three-letter amino acid residues from target PDB files (`6XOX_GLP1R_chainR.pdb`) into a continuous single-letter FASTA sequence required by modern ML architectures.
2. **Combinatorial Ligand Library Generation (`generate_keys.py`):** An algorithmic string-manipulation utility that automates site-specific chemical modifications (including Fluorination and Chlorination) across a library of small-molecule candidates, systematically mutating a core Danuglipron scaffold using SMILES string slicing.
...
![GLP-1R Binding Pocket](results/structure_screenshot_1778206799056.jpg)
