...
1. **Structural Data Ingestion (`scripts/get_seq.py`):** A custom Python parser designed to map and convert three-letter amino acid residues from target PDB files (`data/6XOX_GLP1R_chainR.pdb`) into a continuous single-letter FASTA sequence required by modern ML architectures.
